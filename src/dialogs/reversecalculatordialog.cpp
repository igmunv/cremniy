...
bool ReverseCalculatorDialog::parseValue(const QString &text,
                                         qulonglong *outValue) {
  if (!outValue)
    return false;
  ...
  if (num.startsWith("0x", Qt::CaseInsensitive)) {
    v = num.mid(2).toULongLong(&ok, 16); // hex
  } else if (num.startsWith("0b", Qt::CaseInsensitive)) {
    v = num.mid(2).toULongLong(&ok, 2); // bin
  } else {
    v = num.toULongLong(&ok, 10); // dec
  }
  if (!ok)
    return false;
